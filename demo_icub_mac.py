...
class ICubDemoHandler(Thread):
    ...
    def __init__(self, clf_rate=5, n_acquisition=30, n_jobs=1):
        ...
        self._txt = None
        self._resize = Resize(self._crop)

        # Global property of img and plot, due to UI problems with MacOSX
        self._curr_img = None
        self._curr_plot = None
    ...
    @curr_img.setter
    def curr_img(self, img):
        self._curr_img = img

    @property
    def curr_plot(self):
        return self._curr_plot

    @curr_plot.setter
    def curr_plot(self, plot):
        self._curr_plot = plot

    @classmethod
    def visualize_windows(cls, icube_obj):
        while True:
            if icube_obj.curr_img is not None:
                cv2.imshow('frame', icube_obj.curr_img)
                # print("shown img")
                if cv2.waitKey(1) == 27:
                    break  # esc to quit
            # else:
            #     print('No img found')
            #     time.sleep(1)
            if icube_obj.curr_plot is not None:
                cv2.imshow('scores', icube_obj.curr_plot)
                # print("shown plot")
                if cv2.waitKey(1) == 27:
                    break  # esc to quit
            # else:
    ...
    def _auto_crop(self, perc=PERC_BOX):
        img = self._cap.read()[1]
        self._crop = round(min(img.shape[:2]) * perc) if self._crop is None else self._crop
        return self._crop

    def _show_img(self, img, txt=None):
        self._auto_crop(img)
        # crop = 224
        upp_l, bott_r = self._get_crop_points(img)
    ...
    def _center_crop(self, img):
        # Works with self._crop
        h, w, _ = img.shape
        upp_l, bott_r = self._get_crop_points(img)
        img_cropped = img[upp_l[1]:bott_r[1], upp_l[0]:bott_r[0]]
        img_cropped = cv2.cvtColor(img_cropped, cv2.COLOR_RGB2BGR)
        img_resized = cv2.resize(img_cropped, (CLF_INPUT_DIM, CLF_INPUT_DIM))
        return img_resized

    def _get_crop_points(self, img):
        # Works with self._crop
        h, w, _ = img.shape
        upp_l = (int((w - self._crop) / 2.), int((h - self._crop) / 2.))
        bott_r = (upp_l[0] + self._crop, upp_l[1] + self._crop)
        return upp_l, bott_r
...
if __name__ == '__main__':
    ...
    t = ICubDemoHandler(clf_rate, n_acquisition, n_jobs)
    t.start()
    ICubDemoHandler.visualize_windows(t)
