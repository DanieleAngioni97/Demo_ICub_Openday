...
class ICubDemoHandler(Thread):
    ...
    def _handle_command(self, cmd):
        ...
        self._clf.fit(self._tr_set.X, self._tr_set.Y)
        self._n_classes = len(self._class_names)
    ...
    def _load_clf(self):
        ...
        net = CClassifierPyTorch(model=model, input_shape=(3, 224, 224),
                                 pretrained=True, preprocess=normalizer)
        self._feature_extractor = CNormalizerDNN(net=net, out_layer='relu6')
        ...
        if fm.file_exist(clf_path):
            ...
            self._load_tr_set()
            print("training classifier")
            self._clf = CClassifierSVM_(C=0.1, n_jobs=self._n_jobs)
            self._clf.fit(self._tr_set.X, self._tr_set.Y)
            save(clf_path, self._clf)
            print("classifier trained")
        self._class_names = icub7_names.copy()
        self._n_classes = len(self._class_names)
        self._buffer = CArray.zeros(shape=(self._clf_rate,
    ...
    def _classify(self, img, i):
        img = self._feature_extractor.transform(img)
        label, scores = self._clf.predict(img, return_decision_function=True)
